...
| `create_data_type` | both | `Dealflow Touchpoint` create-if-absent | write |
...
| CRM tools per the five capability slots in [`crm-sync.md`](../skills/dealflow-memory/references/crm-sync.md) | Tier R reads; Tier W sync |
| Conversation-reading messaging tools | Connector tier in [`messaging-capture.md`](../skills/dealflow-memory/references/messaging-capture.md) |
...
Conformance check: exercise each required tool once against a live server (the dated runs in [`testing.md`](testing.md) each did this implicitly; a scripted suite — as built live against the MCP server during the 2026-08-28 session — makes it explicit).
