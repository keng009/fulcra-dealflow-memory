...
> **Pre-release note (this block is removed at v0.3.0):** published releases currently serve v0.2.0 — the previous flow. To try the version described on this page today, zip the skill folders yourself (the install steps show the fallback); v0.3.0 ships once [#31](https://github.com/keng009/fulcra-dealflow-memory/issues/31) and the review gate ([#1](https://github.com/keng009/fulcra-dealflow-memory/issues/1)) close.
...
2. In Claude, open **Customize → Connectors** and connect **Fulcra**. First time, or using an agent harness instead of Claude's app? [Connecting Fulcra](skills/dealflow-memory/references/connect-fulcra.md) has both paths — and points at Fulcra's official `fulcra-get-started` skill, which lets your agent drive the connection.
3. Download `dealflow-demo.zip` from the [latest release](https://github.com/keng009/fulcra-dealflow-memory/releases/latest) *(currently v0.2.0 — see the pre-release note above)* (or zip the `skills/dealflow-demo` folder yourself) and upload it in Claude under **Customize → Skills → + Create skill → Upload a skill**. If Skills isn't visible, enable it under **Settings → Capabilities** first.
...
2. Download `dealflow-memory.zip` from the [latest release](https://github.com/keng009/fulcra-dealflow-memory/releases/latest) *(currently v0.2.0 — see the pre-release note above)* (or zip the `skills/dealflow-memory` folder — its `references/` subfolder must travel inside the zip) and upload it the same way.
3. Say **"show me my last 30 days"**, **"log my call with Jane"**, **"seen this company before?"**, **"prep my day"**, **"what do I owe people"**, **"what moved this week"**, or **"who have I gone quiet on"**.

Only the Fulcra connector is required. If your Fulcra account has calendar data, the skill detects it and uses it: touchpoints get corroborated against real meetings, and "prep my day" reads the actual calendar. If a transcript tool (Otter, Zoom, Fireflies) is connected, it can log meetings straight from transcripts; opt into `email` and the sweep reads your mailbox for real founder conversations too (deck-submission forms, data-room invites and other notifications are surfaced as signals, never logged as conversations). Nothing to configure — each session it states what it found and what connecting more would add.

Want it hands-off? Say **"auto-log my calls"** once (a standing, revocable yes — [ADR-0009](docs/adr/0009-unattended-auto-log.md)): from then on each sweep saves the clear founder calls and meetings without asking, parks the unclear ones for your review, and leaves a receipt and a digest. Then **"make this automatic"** and, where your Claude can schedule things, the skill builds a weekday sweep for you ([how, per harness](skills/dealflow-memory/references/scheduling.md)); where it can't, you just say **"sweep"**. Both were built and live-run in the sales sibling; under this flavor they are designed and not yet live-tested ([docs/testing.md](docs/testing.md)).
...
Keep it. Your CRM is never mirrored into Fulcra: the memory holds the relationships you actually touch — the people you've met with in the last month or two, growing as you work — not a copy of thousands of contacts. (Individual CRM notes about those working-set people can be imported as touchpoints on your say-so — selection, never mirroring.) Breadth questions ("do we know anyone at Acme?") belong to your CRM, which already answers them; the memory answers the questions your CRM can't, about the conversations you actually had. If your Claude has CRM tools connected, `dealflow-memory` offers — once per session, never requires — to copy each logged touchpoint the *other* way, into the CRM as a note on the matched contact, with follow-ups as tasks where the CRM supports them. The sync is one-way, Fulcra → CRM, notes and tasks only: it never creates contacts and never edits fields, stages, or amounts, so your CRM stays the system of record for pipeline. Logging the same touchpoint twice can't duplicate notes — every note title ends with a deterministic key, and the skill scans the contact's existing note titles before writing. Tested with Attio ([test record](docs/testing.md)). Notion and Affinity have official read/write connectors and should work — if you try one, tell us by opening an issue. HubSpot's official Claude connector is now write-capable (it was read-only until mid-2026), so HubSpot sync works through it — live-tested in the sales sibling, untested under this flavor. No CRM connected? The skill never mentions one.
...
These skills are instruction files: they add no backend of their own — no author-operated server, no telemetry, no analytics, and nothing that reports back to the authors. Your requests are still processed by Claude and by Fulcra's hosted service under their own terms and privacy policies. The skills write to exactly two places in your Fulcra account: files under the `/dealflow/` folder, and typed records in the account-level **Dealflow Touchpoint** data type they create — plus, only if you accept the offer, notes in your own CRM. Outside those they only read, and only what the features need: your data-catalog listing (to show you what your account holds), your calendar if connected (to corroborate touchpoints and prep tomorrow), and your timezone — and, when a transcript tool or CRM is connected, the snapshot also reads recent transcripts and CRM notes/meetings in its window to build what it shows you (per-meeting enrichment and the tracked-vs-untracked check). Those snapshot reads happen whether or not you ever accept CRM sync, and they write nothing. Your mailbox is read only if you opt into `email` (or ask for an email sweep): thread headers and bodies in the sweep window, distilled into summaries, never sent, replied to, or labeled ([ADR-0010](docs/adr/0010-email-as-a-source.md)). In hands-off mode ([ADR-0009](docs/adr/0009-unattended-auto-log.md)) writes still go only to those same places, every run leaves a receipt in your own `handoff.md`, and removing one line turns it off. If you accept CRM sync, they additionally search your CRM's contacts and read existing note titles before each write, to dedupe; if you log from a specific transcript, they read the one you pick. They never write credentials, tokens, or secrets to any file, and they never send email or messages on your behalf: ask for a follow-up and you get a clearly labeled draft. File deletes in Fulcra are soft, so removing the demo's sample file is reversible; the sample typed record has no per-record delete through the connector — the demo tells you this **before** offering sample data — and its cleanup tombstones the sample's key, so the record sits inert and excluded from every read these skills perform.
